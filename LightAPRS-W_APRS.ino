// Libraries Geofence, missing Dot at North Korea
...
// #define RAW_GPS // RAW GPS datas. Uncomment to enable for debugging.

//https://github.com/lightaprs/LightAPRS-1.0/wiki/Tips-&-Tricks-for-Pico-Balloons

//****************************************************************************
char  CallSign[7]="NONAME"; //DO NOT FORGET TO CHANGE YOUR CALLSIGN
...
// Todo
char comment[50] = "Pico Balloon"; // Max 50 char
char StatusMessage[50] = "Pico Balloon";
//*****************************************************************************
...
//        Serial.println(F("Sent"));
    ...
    if ((gps.location.age() < 1000 || gps.location.isUpdated()) && gps.location.isValid()) {
      ...
      if (!radioSetup || TxCountFreqbyLocation == 200) {
        configureFreqbyLocation();
		TxCountFreqbyLocation = 1;
      }          
      
      //send status message every 60 minutes
	  if(gps.time.minute() == 0 && gps.time.second() == 30){ 
        setGPS_powersave();
        delay(100);	
...
void updateTelemetry() {
  ...
  telemetry_buff[22] = 'C';
  telemetry_buff[23] = ' ';
  dtostrf(readBatt(), 5, 2, telemetry_buff + 24);
  telemetry_buff[29] = 'V';
  telemetry_buff[30] = ' ';
  sprintf(telemetry_buff + 31, "%02d", gps.satellites.isValid() ? (int)gps.satellites.value() : 0);
  telemetry_buff[33] = 'S';
  telemetry_buff[34] = ' ';
  sprintf(telemetry_buff + 35, "%s", comment);
